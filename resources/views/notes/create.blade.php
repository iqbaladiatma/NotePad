  ...
  <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
    <div class="overflow-hidden shadow-xl bg-white/90 backdrop-blur-sm sm:rounded-3xl animate-fade-in-up">
      <div class="p-8 text-gray-900">
        <div class="mb-8">
          <h2 class="text-3xl font-bold bg-gradient-to-r from-purple-700 to-purple-900 bg-clip-text text-transparent">Create New Note</h2>
          <p class="mt-2 text-gray-600">Capture your thoughts and ideas in a new note.</p>
        </div>
        ...
        <form action="{{ route('notes.store') }}" method="POST" class="space-y-6">
          ...
          <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title') }}"
              class="block w-full px-4 py-3 border border-purple-200 rounded-xl shadow-sm focus:border-purple-500 focus:ring-purple-500 transition-all duration-200"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select name="category" id="category"
                ...
                <option value="tasks" {{ old('category') == 'tasks' ? 'selected' : '' }}>Tasks</option>
              </select>
              @error('category')
              <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
              @enderror
